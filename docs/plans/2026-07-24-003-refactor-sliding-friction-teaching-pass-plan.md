...
在第二轮“课堂控制变量实验页”基础上，继续优化 `web-app` 的滑动摩擦力实验页，把默认体验继续收紧为“统一主流程 + 二级辅助操作”的课堂探究页。

这轮不改技术栈、不改路由、不改 topic 兼容，不回到旧的多模式主导结构。重点是继续收紧教学顺序：延后公式揭示、限制直接跳阶段、让记录门槛更像真实实验、把记录区升级成更明确的课堂实验单。
...
- 顶部步骤仍可直接跳阶段，用户可以绕过观察过程
- 公式与原理在默认界面中出现偏早，实验探究顺序还不够纯粹
- “稳定可记录”主要依赖预设实验阶段，读数稳定的观测感不强
- 记录区虽然已经按因素分组，但还不够像“待完成 / 已完成”的课堂实验单
- 2D 主舞台仍有较多并列信息表达，器材和读数的注意力中心不够强

这轮计划以 `docs/brainstorms/2026-07-24-sliding-friction-lab-teaching-pass-requirements.md` 为上游需求文档，以 `docs/plans/2026-07-24-002-refactor-sliding-friction-classroom-flow-plan.md` 为当前实现基线，继续围绕“统一主流程优先”的目标做收敛。

## Requirements Trace

- R1-R3. 默认课堂入口继续收紧为统一主流程，扩展能力下沉到辅助操作 / 扩展观察区。
...
- `web-app/app/components/control-step-group.tsx`
  - 现有共享控件足够支撑“辅助操作下沉”与“课堂主流程减负”，无需新建一套控件体系
...
- 决策 2：把“主流程”与“辅助操作”分层，而不是引入角色区分
  - 原因：页面只保留一个统一界面，但仍需要把阶段切换、复位等二级操作从主流程里让开
...
- 顶部步骤条是否直接删除：否；默认保留为进度提示，但不再直接点击跳阶段，跳阶段下沉到辅助操作。
- 扩展模式是否继续保留：是；统一下沉到辅助操作 / 扩展观察区，不再与课堂主操作并列。
...
| 层 | 默认定位 | 默认可见内容 | 下沉内容 |
|---|---|---|---|
| 课堂主流程层 | 统一界面默认层 | 当前研究因素、当前组条件、读数稳定提示、记录本组、预制实验单 | 阶段跳转、扩展模式、完整公式 |
| 教学解释层 | 统一界面渐进解释层 | 记录后可展开的“本组原理” | 首屏完整公式、完整结论 |
| 辅助操作层 | 统一界面二级操作层 | 步骤切换、快捷复位、返回主流程 | 不再与主流程主操作并列 |

```mermaid
flowchart TB
  A[课堂默认态] --> B[主流程<br/>变量 -> 观察 -> 稳定 -> 记录]
...
- [x] **Unit 2: 收紧统一主流程入口并下沉辅助操作**

**Goal:** 把当前课堂操作区继续收紧为统一主流程入口，同时把阶段切换、复位与扩展切换下沉到二级辅助操作。
...
**Approach:**
- 默认课堂视图中，顶部步骤条改为只读进度表达或仅高亮当前阶段，不再直接响应点击跳阶段
- 把阶段跳转、返回主流程、复位等二级操作统一收纳到“辅助操作”区，扩展模式保持在“扩展观察”区
...
**Test scenarios:**
- Happy path — 打开实验页时，顶部步骤条只提示当前阶段，点击跳阶段只保留在辅助操作区
- Happy path — 切到扩展观察后，仍可以通过辅助操作返回主流程
- Edge case — 从辅助操作切回主流程时，当前研究因素、已记录数据与主舞台状态保持一致
- Edge case — 扩展模式按钮不再与“开始实验 / 记录本组”并列
- Integration — 全屏、亮暗主题和控制面板折叠后，主流程 / 辅助操作 / 扩展观察三层结构仍清晰可辨
