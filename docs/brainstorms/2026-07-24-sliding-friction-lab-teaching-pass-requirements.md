...
当前 `web-app/app/components/basic-force-lab.tsx` 已经完成了知识点对齐和第二轮课堂流程重排，页面已经从“多模式受力演示页”收敛为“课堂控制变量实验页”。

但从教学体验看，这一页仍然更像“功能面板”，还不够像“顺着就能做完一轮实验的课堂实验单”。

当前主要问题有：

- 顶部步骤仍可直接跳阶段，容易让用户绕过实验过程
- 公式与原理出现得偏早，容易把页面变成“先看答案，再做实验验证”
- 课堂结论仍偏主动输出，留给课堂归纳的空间不够
- “可记录”更像页面预设相位，而不够像真实实验中的“读数稳定”
- 2D 主舞台信息仍偏密，用户容易先看 UI，而不是先看器材、读数和变量

这轮优化的目标不是推翻第二轮，而是把第二轮已经建立的课堂骨架继续教学化：让默认体验更接近“统一主流程 + 二级辅助操作”的课堂探究页。
...
- R2. 顶部步骤条默认只承担“当前进度提示”，不得继续作为可直接跳阶段的主交互。
- R3. 扩展观察能力（3D、恒力拉动、手动拖动、阶段跳转）必须继续保留，但默认下沉到辅助操作或扩展区域，不与课堂主操作并列抢注意力。

**原理揭示时机**
- R4. 默认首屏不得直接完整暴露 `f = μN`、`N = G` 和正式课堂结论。
- R5. 页面应先帮助用户完成至少一组有效测量，再逐步揭示“匀速时拉力等于滑动摩擦力”以及更完整的公式关系。
...
- R9. 页面必须把“当前研究因素”“本组条件”“当前是否可记”表达得比现在更直接，让用户不需要额外解释就能知道下一步做什么。

**结果组织与课堂归纳**
- R10. 记录区应从“已记录的数据展示”继续收敛成“预制课堂实验单”，让用户看到当前因素还差哪几组未完成。
- R11. 课堂结论必须分层表达：未完成对照时不给正式结论，完成部分数据时只给趋势提示，完成足够组数后才给正式归纳。
- R12. 页面应在记录完一组后给出“下一组建议”或“还缺哪组”，帮助用户完成完整控制变量对照。
...
- 首次进入页面后，5 秒内能知道“先改哪个变量、接下来要做什么、什么时候才能记数”。
- 用户在没有额外操作说明的前提下，能够自主完成一轮压力对照的 2-3 组实验。
...
- 默认体验以“统一主流程探究安全”为先，快捷操作可以保留，但必须下沉。
...
- [Affects R2, R3][Product] 顶部步骤条、阶段跳转和扩展模式是统一放入“辅助操作”，还是拆成两个独立的二级入口更清晰。
